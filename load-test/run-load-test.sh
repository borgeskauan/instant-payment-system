...
run_prepared_workload() {
    ...
    local run_started_at_ms run_finished_at_ms status=0 grafana_url
    ...
    run_finished_at_ms="$(date +%s%3N)"
    grafana_url="${GRAFANA_BASE_URL%/}${GRAFANA_DASHBOARD_PATH}?from=${run_started_at_ms}&to=${run_finished_at_ms}"
    printf '%s\n' "$grafana_url" > "$RESULT_DIR/logs/grafana-url.txt"
    log_phase "Grafana dashboard for this run: $grafana_url"
    return "$status"
}
