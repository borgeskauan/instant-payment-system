...
if [[ "$PROFILE_WARMUP_TARGET_TX_RATE" != 50 || "$PROFILE_WARMUP_SECONDS" != 5 || "$PROFILE_WARMUP_COMPLETION_TIMEOUT_SECONDS" != 30 || "$PROFILE_ACTIVE_SECONDS" != 60 || "$PROFILE_DRAIN_SECONDS" != 30 ]]; then
