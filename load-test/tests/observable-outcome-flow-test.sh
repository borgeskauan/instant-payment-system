...
mkdir -p "$tmp_dir/profiles" "$tmp_dir/prepared/current/inputs" "$tmp_dir/prepared/current/certs" "$tmp_dir/bin"
...
"$RUNNER" --no-jfr --no-system-diagnostics observable >/dev/null
cat > "$tmp_dir/expected" <<'EOF'
build
diagnostics run --run-dir RESULT --no-jfr --no-system-diagnostics -- BINARY run --run-dir RESULT --client-cert-root CERTS
