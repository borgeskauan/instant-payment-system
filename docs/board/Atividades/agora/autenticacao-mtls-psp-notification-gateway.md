...
Hoje o stream gRPC entre PSP e `notification-gateway` ainda depende de identidade informada pela própria aplicação, como `Subscribe { ispb }`. Para que o gateway entregue notificações e aceite ACKs com segurança, a identidade do PSP precisa vir da conexão autenticada.
