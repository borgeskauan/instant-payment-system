...
- [x] Isolar DICT, PSP simulado e Angular como uma reference demo sem garantias do core
...
Os três projetos foram movidos para `demo/`, receberam Compose, README, screenshot e smoke próprios, e deixaram de participar do Compose principal. O smoke provou consulta ao DICT, submissão HTTP/2/mTLS ao core, decisão do PSP recebedor pelo protocolo Pull e os dois saldos finais esperados.

Os únicos ajustes funcionais foram bloqueadores do happy path: o PSP foi alinhado ao contrato Pull atual do Gateway e passou a usar HTTP/2 obrigatório no ingresso. Código exclusivo do protocolo push/ACK anterior foi removido. As melhorias opcionais dos três projetos foram reclassificadas em `Backlog/reference-demo/`, e readiness da demo foi retirada da task de readiness do core.
