...
suffix="$(date +%s)"
pix_key="bob-${suffix}@example.com"
...
while ((SECONDS < deadline)); do
  ...
  if jq -en --arg before "$sender_balance" --arg after "$current_sender_balance" --arg amount "25.50" '($before | tonumber) - ($after | tonumber) == ($amount | tonumber)' >/dev/null \
      && jq -en --arg before "$receiver_balance" --arg after "$current_receiver_balance" --arg amount "25.50" '($after | tonumber) - ($before | tonumber) == ($amount | tonumber)' >/dev/null; then
