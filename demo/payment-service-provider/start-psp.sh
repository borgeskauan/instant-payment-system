...
Usage:
  demo/payment-service-provider/start-psp.sh [BANK_CODE] [options]
...
Examples:
  demo/payment-service-provider/start-psp.sh
  demo/payment-service-provider/start-psp.sh 22222222
  demo/payment-service-provider/start-psp.sh --bank-code 33333333 --host-port 8083 --replace
  demo/payment-service-provider/start-psp.sh 44444444 --funds-balance 5000
EOF
}
...
if $dry_run; then
  ...
  if $build_image; then
    quote_cmd docker build -t "$image" "$root/demo/payment-service-provider"
  fi
...
if $build_image; then
  docker build -t "$image" "$root/demo/payment-service-provider"
fi
